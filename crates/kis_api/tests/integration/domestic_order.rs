...
async fn test_domestic_unfilled_orders_vts() {
    ...
    assert!(
        result.is_ok(),
        "domestic_unfilled_orders() failed: {:?}",
        result.err()
    );
}
...
async fn test_domestic_volume_ranking_vts() {
    ...
    let client = kis_api::KisDomesticClient::new(config);
    let result = client
        .domestic_volume_ranking(&kis_api::DomesticExchange::KOSPI, 5)
        .await;
    assert!(
        result.is_ok(),
        "domestic_volume_ranking() failed: {:?}",
        result.err()
    );
}
...
async fn test_domestic_order_tr_ids_vts() {
    ...
    assert!(
        result.is_ok(),
        "domestic_balance() failed: {:?}",
        result.err()
    );
    let balance = result.unwrap();
    println!(
        "domestic balance items: {}, purchase_amount: {}",
        balance.items.len(),
        balance.summary.purchase_amount
    );
}
...
async fn test_domestic_order_history_vts() {
    ...
    let client = kis_api::KisDomesticClient::new(config);
    let result = client
        .domestic_order_history(kis_api::DomesticOrderHistoryRequest {
