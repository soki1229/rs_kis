...
async fn test_check_deposit_vts() {
    ...
    let result =
        kis_api::rest::overseas::deposit::check_deposit(kis.shared_http(), &config, &token).await;
...
async fn test_symbol_info_real() {
    ...
    assert!(
        !info.name.is_empty(),
        "name should not be empty (got: {:?})",
        info
    );
    println!(
        "symbol_info: name={}, exchange={}",
